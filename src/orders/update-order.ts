...
export const updateOrderHandlerFactory = (
    {
        dynamo,
        tableName
    }: UpdateOrderHandlerDependencies
): UpdateOrderHandler => async (id, updates) => {
    try {
...
const serializeUpdates = (updates: OrderUpdate[]): SerializedUpdate => {
...
const mapToIntermediate = (update: OrderUpdate, index: number): UpdateIntermediate<'status'> => {
...
const keyFromIndex = (index: number): string => `${index.toString().split('').map(encodeDigit).join()}`;

const encodeDigit = (num: Digit): string => ({
    '0': 'a',
    '1': 'b',
    '2': 'c',
    '3': 'b',
